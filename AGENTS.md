...
`GhosttySurfaceView` (an NSView) handles `mouseDown` before SwiftUI's gesture system sees the event, so the `.onTapGesture { store.focus(paneID) }` on `PaneView`'s ZStack does NOT fire for clicks on the terminal surface. To propagate focus changes from clicks back to the store, every terminal-surface `mouseDown` posts a `glintPaneFocusClicked` notification (same pattern as Esc/Return) with the pane key. Do not gate this notification on `window.firstResponder`: AppKit's responder and `WorkspaceStore.focusedPane` can briefly disagree while queued SwiftUI responder updates unwind after a split, and dropping the click in that state pins focus to the most recently opened pane. The store observes the notification **synchronously** (`queue: nil` + `MainActor.assumeIsolated`) so that `focusedPane` updates before the next `updateNSView` pass can fight back and resign the first responder; redundant and stale notifications are ignored. Do NOT rely on `.onTapGesture` alone for focus — it only fires for clicks on SwiftUI-rendered areas, not on NSViewRepresentable content. Keep the responder-drift regression covered in `SplitFocusDimTests.testSurfaceClickNotifiesWhenSurfaceIsAlreadyFirstResponder`.
