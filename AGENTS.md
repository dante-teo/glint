...
`ACPClient` routes newline-delimited JSON-RPC in one background stdout reader: responses resolve pending continuations by `id` (via `PendingRequest`, a lock-guarded single-resume wrapper — see below), notifications publish through `notifications`, and server requests dispatch to registered async handlers that write JSON-RPC results or errors. Malformed JSON lines are logged and ignored. Process termination, EOF, timeout, or cancellation fail affected pending continuations. Each `request(method:params:timeout:)` call races its own `timeoutTask` against the response (default 60s; `session/close` uses 2s) — do not add a second, redundant timeout layer on top of a single ACP call; if a caller needs a hard wall-clock bound across *multiple* concurrent calls, race an unstructured `Task` against a timeout `Task` and resume a shared continuation exactly once (see `WorkspaceStore.ResumeOnce`, which mirrors `ACPClient.PendingRequest`'s pattern) — a `withTaskGroup` alone does not bound execution time, since cancelling a task group's children only sets a flag and the group still awaits all of them before returning.
