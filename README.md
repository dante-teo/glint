...
The app bundle icon is a conventional macOS `AppIcon.appiconset` under `Glint/Resources/Assets.xcassets`; selectable Dock icon presets are checked-in, approved asset-catalog renders. Private source photographs and intermediate icon artwork are intentionally excluded from the repository. See [docs/appearance.md](docs/appearance.md#app-icon-packaging) before changing app icon resources.
