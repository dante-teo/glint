...
For the canonical base portrait, face likeness matters more than clothing or hair detail. Keep `source-photo.png` as the closest useful square crop for facial identity, and keep `lineart-master.png` as a polished line-art master derived from that crop. If an AI image-editing pass is used to clean the master, constrain it to preserve the face geometry, eye spacing, nose, jaw, and expression. Lips should stay understated: avoid hard outer lip outlines, dense vertical lip texture, and tooth-like lines, because those become abrupt after alpha extraction and theme coloring. Hair can carry more line density, but it should read as smooth flow and intentional dark masses rather than scratchy texture.
